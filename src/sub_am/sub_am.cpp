 ...
 *
 * Amplitude modulation effect using the sub timbre as modulator and the main timbre as carrier
 *
 */
                   ...
                   uint32_t frames)
{
  ...
  for (; my != my_e; ) {
    ...
    const float output_left = main_left*(1.f + clip1m1f(sub_left*scale))*depth;
    const float output_right = main_right*(1.f + clip1m1f(sub_right*scale))*depth;
